...
const Header = () => {
  ...
  return (
    <>
<header
          className="ud-header left-0 top-0 z-50 flex w-full items-center shadow-nav fixed border-b border-stroke bg-white backdrop-blur-[5px]"
        >
...
<button
                 ...
                 >
<span className={`relative my-1.5 block h-0.5 w-[30px] transition-all duration-300 ${navbarOpen ? " top-[7px] rotate-45" : " "}`} />
                    <span className={`relative my-1.5 block h-0.5 w-[30px] transition-all duration-300 ${navbarOpen ? "opacity-0 " : " "}`} />
                    <span className={`relative my-1.5 block h-0.5 w-[30px] transition-all duration-300 ${navbarOpen ? " top-[-8px] -rotate-45" : " "}`} />
                 </button>
<nav
                    id="navbarCollapse"
...
{/* Always use dark text for navigation links */}
<Link
  onClick={navbarToggleHandler}
  scroll={false}
  href={menuItem.path}
...
{/* Always use light theme for header links since navbar is always white */}
<>
  <Link
